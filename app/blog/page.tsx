    ...
    <aside className="space-y-6">
      ...
      <div className="rounded-2xl bg-gray-50 border border-gray-100 p-5">
        <div className="flex items-center gap-3">
          ...
          />
          <button className="h-11 w-11 rounded-xl bg-[#1c2d56] text-white font-bold hover:bg-[#1c2d56]/90">
            ⌕
          </button>
        </div>
      </div>
      ...
      <div className="rounded-2xl bg-gray-50 border border-gray-100 p-5">
        ...
        <div className="mt-4 space-y-4">
          {recent.map((b) => (
            <Link key={b.slug} href={`/blog/${b.slug}`} className="grid grid-cols-[54px_1fr] gap-3 items-center">
              <Image
                src={b.image}
                alt={b.title}
                width={108}
              ...
              />
              <div>
                <div className="text-sm font-bold text-slate-900 leading-snug line-clamp-2">{b.title}</div>
      ...
      <div className="rounded-2xl bg-gray-50 border border-gray-100 p-5">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {tags.map((t) => (
            <span key={t} className="rounded-lg bg-white border border-gray-200 px-3 py-1 text-xs text-gray-700">
              {t}
            </span>
          ))}
        </div>
      ...
      <div className="rounded-2xl bg-gray-50 border border-gray-100 p-5">
        ...
        <input placeholder="Type your email..." className="mt-4 w-full rounded-xl bg-white border border-gray-200 px-4 py-3 text-sm outline-none" />
        <button className="mt-3 w-full rounded-xl bg-[#1c2d56] hover:bg-[#1c2d56]/90 py-3 text-sm font-bold text-white">
          Subscribe
        </button>
      </div>
    </aside>
...
function Pagination({ page, totalPages }: { page: number; totalPages: number }) {
  return (
    <div className="flex items-center gap-3 justify-center pt-8">
      <Link href={`/blog?page=${Math.max(1, page - 1)}`} className={`rounded-xl border px-4 py-2 text-sm font-semibold ${page <= 1 ? "pointer-events-none opacity-50" : "hover:bg-gray-50"}`}>
        Prev
      </Link>
      <div className="text-sm text-gray-600">
        Page <span className="font-bold text-slate-900">{page}</span> of <span className="font-bold text-slate-900">{totalPages}</span>
      </div>
      <Link href={`/blog?page=${Math.min(totalPages, page + 1)}`} className={`rounded-xl border px-4 py-2 text-sm font-semibold ${page >= totalPages ? "pointer-events-none opacity-50" : "hover:bg-gray-50"}`}>
        Next
      </Link>
    </div>
  );
...
export default async function BlogPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
            ...
            <article key={blog.slug} className="space-y-4">
              <Link href={`/blog/${blog.slug}`} className="block">
                <div className="overflow-hidden rounded-2xl">
                  <Image src={blog.image} alt={blog.title} width={1200} height={650} className="w-full h-[280px] md:h-[320px] object-cover" />
                </div>
              </Link>
  ...
  {blog.caption}
              ...
              <Link href={`/blog/${blog.slug}`} className="inline-flex items-center justify-center rounded-md bg-[#1c2d56] px-6 py-3 text-sm font-bold text-white hover:bg-[#1c2d56]/90">
                Read More
              </Link>
            </article>
          ))}
